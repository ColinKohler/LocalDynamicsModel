...
class ADNBoxPalletizingConfig(Config):
  def __init__(self, num_gpus=1, results_path=None):
    super(ADNBoxPalletizingConfig, self).__init__(num_gpus=num_gpus)
    self.seed = 0

    # Env
    self.env_type = 'box_palletizing'
    self.expert_env = 'box_palletizing'
    self.use_rot = True
    ...
    # Training
    self.root_path = os.path.join(constants.RESULTS_PATH, 'box_palletizing')

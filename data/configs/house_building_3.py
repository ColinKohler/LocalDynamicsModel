...
class ADNHouseBuilding3Config(Config):
  def __init__(self, num_gpus=1, results_path=None):
    super(ADNHouseBuilding3Config, self).__init__(num_gpus=num_gpus)
    self.seed = 0

    # Env
    self.env_type = 'house_building_3'
    self.expert_env = 'house_building_3_deconstruct'
    self.use_rot = True
    ...
    self.obs_size = constants.HOUSE_BUILDING_3_ENV_CONFIG['obs_size']
    self.hand_obs_size = constants.HOUSE_BUILDING_3_ENV_CONFIG['in_hand_size']
    self.max_steps = constants.HOUSE_BUILDING_3_ENV_CONFIG['max_steps']
    ...
    # Training
    self.root_path = os.path.join(constants.RESULTS_PATH, 'house_building_3')
    if results_path:

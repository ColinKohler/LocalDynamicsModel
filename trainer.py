...
class Trainer(object):
  def __init__(self, initial_checkpoint, config):
    ...
    if self.config.agent == 'adn':
      ...
      self.agent = ShootingAgent(self.config, self.device, training=True)
    elif self.config.agent == 'dyna':
      self.agent = DynaAgent(self.config, self.device, training=True)
    elif self.config.agent == 'fc_dqn':
      self.agent = FCDQNAgent(self.config, self.device, training=True)
    elif self.config.agent == 'rot_fc_dqn':
      self.agent = RotFCDQNAgent(self.config, self.device, training=True)
    else:
      raise ValueError
  ...
  def continuousUpdateWeights(self, replay_buffer, shared_storage):
    ...
    while self.training_step < self.config.training_steps and \
      ...
      if self.config.agent == 'fc_dqn' or self.config.agent =='rot_fc_dqn':
        priorities, q_value_loss = self.agent.updateWeights(batch)
        state_value_loss = 0
        reward_loss = 0
        forward_loss = 0
        pred_obs = None
      elif self.config.agent == 'shooting':
        priorities, forward_loss, reward_loss = self.agent.updateWeights(batch, class_weight)
        state_value_loss = 0
        q_value_loss = 0
        pred_obs = None
      else:
      ...
      if self.config.agent == 'adn' and self.training_step % self.config.decay_action_sample_pen == 0 and self.training_step > 0:
  ...
  def continuousUpdateWeightsDyna(self, replay_buffer, sim_replay_buffer, shared_storage):
    while ray.get(shared_storage.getInfo.remote('num_eps')) <= 5:
      time.sleep(0.1)
    ...
    while self.training_step < self.config.training_steps and \
      ...
      priorities, q_value_loss, forward_loss, reward_loss = self.agent.updateWeights(batch, class_weight)
      replay_buffer.updatePriorities.remote(priorities, idx_batch)
      state_value_loss = 0
      pred_obs = None

      if self.training_step > self.config.checkpoint_interval:
        next_sim_batch = sim_replay_buffer.sample.remote(shared_storage)
        for _ in range(self.config.num_dyna_updates):
          idx_sim_batch, sim_class_weight, sim_batch = ray.get(next_sim_batch)
          sim_next_batch = sim_replay_buffer.sample.remote(shared_storage)

          _, _, _, _ = self.agent.updateWeights(sim_batch, sim_class_weight, dyna_update=True)
